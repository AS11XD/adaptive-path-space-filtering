...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
bool ImGui::SliderBehaviorRangeT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v_dat, size_t v_size, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb, size_t* nearestIndex)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    ...
    if (g.ActiveId == id)
    {
        ...
        {
            *nearestIndex = 0;
            float distance = fabs(clicked_t - ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, *v_dat, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize));
            for (size_t i = 1; i < v_size; ++i)
            {
                TYPE* v = v_dat + i;
                float tmpDistance = fabs(clicked_t - ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, *v, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize));
    ...
    for (size_t i = 0; i < v_size; ++i)
    {
        ...
        if (slider_sz < 1.0f)
        {
            out_grab_bb[i] = ImRect(bb.Min, bb.Min);
        }
        else
        {
            // Output grab position so it can be displayed by the caller
            ...
            const float grab_pos = ImLerp(slider_usable_pos_min, slider_usable_pos_max, grab_t);
            if (axis == ImGuiAxis_X)
                out_grab_bb[i] = ImRect(grab_pos - grab_sz * 0.5f, bb.Min.y + grab_padding, grab_pos + grab_sz * 0.5f, bb.Max.y - grab_padding);
            else
                out_grab_bb[i] = ImRect(bb.Min.x + grab_padding, grab_pos - grab_sz * 0.5f, bb.Max.x - grab_padding, grab_pos + grab_sz * 0.5f);
        }
    }
...
// So e.g. an integer Slider between INT_MAX-10 and INT_MAX will fail, but an integer Slider between INT_MAX/2-10 and INT_MAX/2 will be ok.
// It would be possible to lift that limitation with some work but it doesn't seem to be worth it for sliders.
bool ImGui::SliderBehaviorRange(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* p_v, size_t v_size, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb, size_t* nearestIdx)
{
    // Read imgui.cpp "API BREAKING CHANGES" section for 1.78 if you hit this assert.
    IM_ASSERT((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) && "Invalid ImGuiSliderFlags flag!  Has the 'float power' argument been mistakenly cast to flags? Call function with ImGuiSliderFlags_Logarithmic flags instead.");
    ...
    switch (data_type)
    {
        case ImGuiDataType_S8:
            {
                ImS32* v32 = new ImS32[v_size];
                for (size_t i = 0; i < v_size; ++i)
                {
                    v32[i] = (ImS32) * ((ImS8*)p_v + i);
                }
                bool r = SliderBehaviorRangeT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, v32, v_size, *(const ImS8*)p_min, *(const ImS8*)p_max, format, flags, out_grab_bb, nearestIdx);
                if (r)
                {
                    for (size_t i = 0; i < v_size; ++i)
                    {
                        *((ImS8*)p_v + i) = (ImS8)v32[i];
                    }
                }
                delete(v32);
                return r;
        ...
        case ImGuiDataType_U8:
            {
                ImU32* v32 = new ImU32[v_size];
                for (size_t i = 0; i < v_size; ++i)
                {
                    v32[i] = (ImU32) * ((ImU8*)p_v + i);
                }
                bool r = SliderBehaviorRangeT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, v32, v_size, *(const ImU8*)p_min, *(const ImU8*)p_max, format, flags, out_grab_bb, nearestIdx);
                if (r)
                {
                    for (size_t i = 0; i < v_size; ++i)
                    {
                        *((ImU8*)p_v + i) = (ImU8)v32[i];
                    }
                }
                delete(v32);
                return r;
        ...
        case ImGuiDataType_S16:
            {
                ...
                for (size_t i = 0; i < v_size; ++i)
                {
                    v32[i] = (ImS32) * ((ImS16*)p_v + i);
                }
                bool r = SliderBehaviorRangeT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, v32, v_size, *(const ImS16*)p_min, *(const ImS16*)p_max, format, flags, out_grab_bb, nearestIdx);
                if (r)
                {
                    for (size_t i = 0; i < v_size; ++i)
                    {
                        *((ImS16*)p_v + i) = (ImS16)v32[i];
                    }
                }
                delete(v32);
                return r;
        ...
        case ImGuiDataType_U16:
            {
                ...
                for (size_t i = 0; i < v_size; ++i)
                {
                    v32[i] = (ImU32) * ((ImU16*)p_v + i);
                }
                bool r = SliderBehaviorRangeT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, v32, v_size, *(const ImU16*)p_min, *(const ImU16*)p_max, format, flags, out_grab_bb, nearestIdx);
                if (r)
                {
                    for (size_t i = 0; i < v_size; ++i)
                    {
                        *((ImU16*)p_v + i) = (ImU16)v32[i];
                    }
                }
                delete(v32);
                return r;
        ...
        case ImGuiDataType_S32:
            IM_ASSERT(*(const ImS32*)p_min >= IM_S32_MIN / 2 && *(const ImS32*)p_max <= IM_S32_MAX / 2);
            return SliderBehaviorRangeT<ImS32, ImS32, float >(bb, id, data_type, (ImS32*)p_v, v_size, *(const ImS32*)p_min, *(const ImS32*)p_max, format, flags, out_grab_bb, nearestIdx);
        case ImGuiDataType_U32:
            IM_ASSERT(*(const ImU32*)p_max <= IM_U32_MAX / 2);
            return SliderBehaviorRangeT<ImU32, ImS32, float >(bb, id, data_type, (ImU32*)p_v, v_size, *(const ImU32*)p_min, *(const ImU32*)p_max, format, flags, out_grab_bb, nearestIdx);
        case ImGuiDataType_S64:
            IM_ASSERT(*(const ImS64*)p_min >= IM_S64_MIN / 2 && *(const ImS64*)p_max <= IM_S64_MAX / 2);
            return SliderBehaviorRangeT<ImS64, ImS64, double>(bb, id, data_type, (ImS64*)p_v, v_size, *(const ImS64*)p_min, *(const ImS64*)p_max, format, flags, out_grab_bb, nearestIdx);
        case ImGuiDataType_U64:
            IM_ASSERT(*(const ImU64*)p_max <= IM_U64_MAX / 2);
            return SliderBehaviorRangeT<ImU64, ImS64, double>(bb, id, data_type, (ImU64*)p_v, v_size, *(const ImU64*)p_min, *(const ImU64*)p_max, format, flags, out_grab_bb, nearestIdx);
        case ImGuiDataType_Float:
            IM_ASSERT(*(const float*)p_min >= -FLT_MAX / 2.0f && *(const float*)p_max <= FLT_MAX / 2.0f);
            return SliderBehaviorRangeT<float, float, float >(bb, id, data_type, (float*)p_v, v_size, *(const float*)p_min, *(const float*)p_max, format, flags, out_grab_bb, nearestIdx);
        case ImGuiDataType_Double:
            IM_ASSERT(*(const double*)p_min >= -DBL_MAX / 2.0f && *(const double*)p_max <= DBL_MAX / 2.0f);
            return SliderBehaviorRangeT<double, double, double>(bb, id, data_type, (double*)p_v, v_size, *(const double*)p_min, *(const double*)p_max, format, flags, out_grab_bb, nearestIdx);
        case ImGuiDataType_COUNT: break;
    }
    IM_ASSERT(0);
    return false;
...
IMGUI_API bool ImGui::SliderScalarRange(const char* label, ImGuiDataType data_type, void* p_data, size_t p_data_count, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (axis == ImGuiAxis_X)
    {
        float sizeYDir = (total_bb.Max.y - total_bb.Min.y) * 0.5f;
        total_bb.Min.y -= sizeYDir;
        total_bb.Max.y += sizeYDir;
        frame_bb.Min.y += sizeYDir * 2.0f;
        frame_bb.Max.y += sizeYDir * 2.0f;
    }
    else
    {
        float sizeXDir = (total_bb.Max.x - total_bb.Min.x) * 0.5f;
        total_bb.Min.x -= sizeXDir;
        total_bb.Max.x += sizeXDir;
        frame_bb.Min.x += sizeXDir * 2.0f;
        frame_bb.Max.x += sizeXDir * 2.0f;
    }
    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id, &frame_bb, temp_input_allowed ? ImGuiItemFlags_Inputable : 0))
    ...
    // Slider behavior
    ImRect* grab_bb = new ImRect[p_data_count];
    size_t nearestIdx;
    bool value_changed = SliderBehaviorRange(frame_bb, id, data_type, p_data, p_data_count, p_min, p_max, format, flags, grab_bb, &nearestIdx);
    if (value_changed)
        MarkItemEdited(id);

    ImVec4* col = new ImVec4[p_data_count + 1];
    for (size_t i = 0; i < p_data_count + 1; ++i)
    {
        size_t rand = 1024 + i;
        float r = (rand * 0.1031f) - floor(rand * 0.1031f);
        float g = (rand * 0.1030f) - floor(rand * 0.1030f);
        float b = (rand * 0.1173f) - floor(rand * 0.1173f);
        float dot = r * (g + 33.33f) + g * (b + 33.33f) + b * (r + 33.33f);
        r += dot;
        g += dot;
        b += dot;
        r = (r + g) * b;
    ...
    for (size_t i = 0; i < p_data_count + 1; ++i)
    {
        if (i == 0)
        {
            if (grab_bb[0].Max.x > grab_bb[0].Min.x && frame_bb.Max.x > frame_bb.Min.x)
            {
                ImVec2 start = frame_bb.Min;
                ImVec2 end;
                if (axis == ImGuiAxis_X)
                {
                    start.x += 4.0f;
                    start.y = frame_bb.Min.y + (frame_bb.Max.y - frame_bb.Min.y) / 2.0f - lineSize * 0.5f;
                    end.x = grab_bb[0].Min.x + (grab_bb[0].Max.x - grab_bb[0].Min.x) / 2.0f;
                    end.y = frame_bb.Min.y + (frame_bb.Max.y - frame_bb.Min.y) / 2.0f + lineSize * 0.5f;
                }
                else
                {
                    start.y += 4.0f;
                    start.x = frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x) / 2.0f - lineSize * 0.5f;
                    end.y = grab_bb[0].Min.y + (grab_bb[0].Max.y - grab_bb[0].Min.y) / 2.0f;
                    end.x = frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x) / 2.0f + lineSize * 0.5f;
                }
                window->DrawList->AddRectFilled(start, end, GetColorU32(col[i]), style.GrabRounding);
            }
        }
        else if (i == p_data_count)
        {
            if (grab_bb[p_data_count - 1].Max.x > grab_bb[p_data_count - 1].Min.x && frame_bb.Max.x > frame_bb.Min.x)
            {
                ImVec2 start = grab_bb[p_data_count - 1].Min;
                ImVec2 end = frame_bb.Max;
                if (axis == ImGuiAxis_X)
                {
                    start.x += (grab_bb[p_data_count - 1].Max.x - grab_bb[p_data_count - 1].Min.x) / 2.0f;
                    start.y = frame_bb.Min.y + (frame_bb.Max.y - frame_bb.Min.y) / 2.0f - lineSize * 0.5f;
                    end.x -= 4.0f;
                    end.y = frame_bb.Min.y + (frame_bb.Max.y - frame_bb.Min.y) / 2.0f + lineSize * 0.5f;
                }
                ...
                {
                    start.y += (grab_bb[p_data_count - 1].Max.y - grab_bb[p_data_count - 1].Min.y) / 2.0f;
                    start.x = frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x) / 2.0f - lineSize * 0.5f;
                    end.y -= 4.0f;
                    end.x = frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x) / 2.0f + lineSize * 0.5f;
                }
        ...
        {
            if (grab_bb[i - 1].Max.x > grab_bb[i - 1].Min.x && grab_bb[i].Max.x > grab_bb[i].Min.x && frame_bb.Max.x > frame_bb.Min.x)
            {
                ImVec2 start = grab_bb[i - 1].Min;
                ImVec2 end = grab_bb[i].Max;
                if (axis == ImGuiAxis_X)
                {
                    start.x += (grab_bb[i - 1].Max.x - grab_bb[i - 1].Min.x) / 2.0f;
                    start.y = frame_bb.Min.y + (frame_bb.Max.y - frame_bb.Min.y) / 2.0f - lineSize * 0.5f;
                    end.x -= (grab_bb[i].Max.x - grab_bb[i].Min.x) / 2.0f;
                    end.y = frame_bb.Min.y + (frame_bb.Max.y - frame_bb.Min.y) / 2.0f + lineSize * 0.5f;
                }
                else
                {
                    start.y += (grab_bb[i - 1].Max.y - grab_bb[i - 1].Min.y) / 2.0f;
                    start.x = frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x) / 2.0f - lineSize * 0.5f;
                    end.y -= (grab_bb[i].Max.y - grab_bb[i].Min.y) / 2.0f;
                    end.x = frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x) / 2.0f + lineSize * 0.5f;
                }
                window->DrawList->AddRectFilled(start, end, GetColorU32(col[i]), style.GrabRounding);
            }
    ...
    // Render grab
    for (size_t i = 0; i < p_data_count; ++i)
    {
        if (grab_bb[i].Max.x > grab_bb[i].Min.x)
        {
            ImU32 interpolCol = GetColorU32(ImVec4((col[i].x + col[i + 1].x) * 0.5f, (col[i].y + col[i + 1].y) * 0.5f, (col[i].z + col[i + 1].z) * 0.5f, 1.0f));
            ImU32 interpolColActive = GetColorU32(ImVec4((col[i].x + col[i + 1].x) * 0.5f + 0.3f, (col[i].y + col[i + 1].y) * 0.5f + 0.3f, (col[i].z + col[i + 1].z) * 0.5f + 0.3f, 1.0f));
            window->DrawList->AddCircleFilled(grab_bb[i].Min + (grab_bb[i].Max - grab_bb[i].Min) * 0.5f, 6.0f,
                                              (g.ActiveId == id && nearestIdx == i) ? interpolColActive : interpolCol, 16);
        }
    }
    delete(col);
    ...
    for (size_t i = 0; i < p_data_count + 1; ++i)
    {
        void* os_p_data = (void*)(byte_offset * i + (uint8_t*)p_data);
        char value_buf[64];
        const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, os_p_data, format);

        ImRect n_frame_bb = frame_bb;
        if (axis == ImGuiAxis_X)
        {
            //n_frame_bb.Min.x = frame_bb.Min.x + ((frame_bb.Max.x - frame_bb.Min.x) * i) / p_data_count;
            //n_frame_bb.Max.x = frame_bb.Min.x + ((frame_bb.Max.x - frame_bb.Min.x) * (i + 1)) / p_data_count;
            if (i == p_data_count)
            {
                n_frame_bb.Min.x = frame_bb.Max.x;
                n_frame_bb.Max.x = frame_bb.Max.x + 2.0f;
            }
            else
            {
                const ImVec2 text_size = CalcTextSize(value_buf, value_buf_end, true);
                n_frame_bb.Min.x = grab_bb[i].Min.x - text_size.x * 0.5f;
                n_frame_bb.Max.x = grab_bb[i].Max.x + text_size.x * 0.5f;
                if (n_frame_bb.Min.x < frame_bb.Min.x)
                {
                    n_frame_bb.Min.x = frame_bb.Min.x;
                    n_frame_bb.Max.x = n_frame_bb.Min.x + text_size.x;
                }
                if (n_frame_bb.Max.x > frame_bb.Max.x)
                {
                    n_frame_bb.Max.x = frame_bb.Max.x;
                    n_frame_bb.Min.x = n_frame_bb.Max.x - text_size.x;
                }
                n_frame_bb.Min.y -= frame_bb.Max.y - frame_bb.Min.y;
                n_frame_bb.Max.y -= frame_bb.Max.y - frame_bb.Min.y;
            }
        }
        else
        {
            if (i == p_data_count)
            {
                n_frame_bb.Min.y = frame_bb.Max.y;
                n_frame_bb.Max.y = frame_bb.Max.y + 2.0f;
            }
            else
            {
                const ImVec2 text_size = CalcTextSize(value_buf, value_buf_end, true);
                n_frame_bb.Min.y = grab_bb[i].Min.y - text_size.y * 0.5f;
                n_frame_bb.Max.y = grab_bb[i].Max.y + text_size.y * 0.5f;
                if (n_frame_bb.Min.y < frame_bb.Min.y)
                {
                    n_frame_bb.Min.y = frame_bb.Min.y;
                    n_frame_bb.Max.y = n_frame_bb.Min.y + text_size.y;
                }
                if (n_frame_bb.Max.y > frame_bb.Max.y)
                {
                    n_frame_bb.Max.y = frame_bb.Max.y;
                    n_frame_bb.Min.y = n_frame_bb.Max.y - text_size.y;
                }
                n_frame_bb.Min.x -= frame_bb.Max.x - frame_bb.Min.x;
                n_frame_bb.Max.x -= frame_bb.Max.x - frame_bb.Min.x;
            }
        }
        ...
        if (i != p_data_count)
            RenderTextClipped(n_frame_bb.Min, n_frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));

        if (label_size.x > 0.0f)
            RenderText(ImVec2(n_frame_bb.Max.x + style.ItemInnerSpacing.x, n_frame_bb.Min.y + style.FramePadding.y), i == p_data_count ? label : "");
    }
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    delete(grab_bb);
    return value_changed;
...
IMGUI_API bool ImGui::SliderScalarRange(const char* label, ImGuiDataType data_type, void* p_data, ImU32* colors, size_t p_data_count, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    for (size_t i = 0; i < p_data_count + 1; ++i)
    {
        if (i == 0)
        {
            if (grab_bb[0].Max.x > grab_bb[0].Min.x && frame_bb.Max.x > frame_bb.Min.x)
            {
                ...
                window->DrawList->AddRectFilled(start, end, colors[i], style.GrabRounding);
            }
        }
        else if (i == p_data_count)
        {
    ...
    for (size_t i = 0; i < p_data_count; ++i)
    {
        if (grab_bb[i].Max.x > grab_bb[i].Min.x)
        {
            uint32_t cr = (colors[i] >> 0) & 0x000000ff;
            uint32_t cg = (colors[i] >> 8) & 0x000000ff;
            uint32_t cb = (colors[i] >> 16) & 0x000000ff;
            uint32_t ca = (colors[i] >> 24) & 0x000000ff;
            uint32_t cr1 = (colors[i + 1] >> 0) & 0x000000ff;
            uint32_t cg1 = (colors[i + 1] >> 8) & 0x000000ff;
            uint32_t cb1 = (colors[i + 1] >> 16) & 0x000000ff;
            uint32_t ca1 = (colors[i + 1] >> 24) & 0x000000ff;

            cr = (cr + cr1) / 2;
            cg = (cg + cg1) / 2;
            cb = (cb + cb1) / 2;
            ca = (ca + ca1) / 2;
            ImU32 interpolCol = 0x0;
            interpolCol |= cr << 0;
            interpolCol |= cg << 8;
            interpolCol |= cb << 16;
            interpolCol |= ca << 24;

            cr = (cr + 0x4d > 0xff) ? 0xff : (cr + 0x4d);
            cg = (cg + 0x4d > 0xff) ? 0xff : (cg + 0x4d);
            cb = (cb + 0x4d > 0xff) ? 0xff : (cb + 0x4d);
            ca = (ca + 0x4d > 0xff) ? 0xff : (ca + 0x4d);
            ImU32 interpolColActive = 0x0;
            interpolColActive |= cr << 0;
            interpolColActive |= cg << 8;
            interpolColActive |= cb << 16;
            interpolColActive |= ca << 24;
            window->DrawList->AddCircleFilled(grab_bb[i].Min + (grab_bb[i].Max - grab_bb[i].Min) * 0.5f, 6.0f,
                                              (g.ActiveId == id && nearestIdx == i) ? interpolColActive : interpolCol, 16);
        }
    }
...
IMGUI_API void ImGui::ColorGradientBar(const char* label, size_t count, ImU32* colors, ImU32* values)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    float sizeY = total_bb.Max.y - total_bb.Min.y;
    total_bb.Max.y += sizeY;
    frame_bb.Min.y += sizeY;
    frame_bb.Max.y += sizeY;
    ...
    // Draw frame
    RenderNavHighlight(frame_bb, id);
    ImRect b_frame_bb = frame_bb;
    float sizeX = (frame_bb.Max.x - frame_bb.Min.x);
    float scalaSize = float(values[count - 1] - values[0]);
    for (size_t i = 0; i < count - 1; ++i)
    {
        b_frame_bb.Min.x = frame_bb.Min.x + (float(values[i] - values[0])) * sizeX / scalaSize;
        b_frame_bb.Max.x = frame_bb.Min.x + (float(values[i + 1] - values[0])) * sizeX / scalaSize;
        window->DrawList->AddRectFilledMultiColor(b_frame_bb.Min, b_frame_bb.Max, colors[i], colors[i + 1], colors[i + 1], colors[i]);
    }

    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");

    float minimumGapSize = (frame_bb.Max.x - frame_bb.Min.x);
    float prevTextEnd = 0.0f;
    std::vector<float> gapPosX(count - 1);
    std::vector<float> gapSizeX(count - 1);
    for (size_t i = 0; i < count; ++i)
    {
        ImRect n_frame_bb = frame_bb;
        n_frame_bb.Min.y -= sizeY;
        n_frame_bb.Max.y -= sizeY;
        char value_buf[64];
        const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), ImGuiDataType_U32, (values + i), "%i");
        ImGui::PushStyleColor(ImGuiCol_Text, colors[i]);
        RenderTextClipped(n_frame_bb.Min, n_frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2((values[i] - values[0]) / scalaSize, 0.5f));
        ImGui::PopStyleColor();
        float curTextSize = CalcTextSize(value_buf, value_buf_end, NULL, false).x;
        float posInFrame = ((values[i] - values[0]) / scalaSize) * (frame_bb.Max.x - frame_bb.Min.x);
        if (posInFrame > (frame_bb.Max.x - frame_bb.Min.x) - curTextSize * 0.5f)
            posInFrame = (frame_bb.Max.x - frame_bb.Min.x) - curTextSize * 0.5f;
        else if (posInFrame < curTextSize * 0.5f)
            posInFrame = curTextSize * 0.5f;

        float curGapSize = posInFrame - curTextSize * 0.5f - prevTextEnd;
        if (i > 0)
        {
            minimumGapSize = (curGapSize < minimumGapSize) ? curGapSize : minimumGapSize;
            gapPosX[i - 1] = frame_bb.Min.x + (prevTextEnd + posInFrame - curTextSize * 0.5f) * 0.5f;
            gapSizeX[i - 1] = curGapSize;
        }
        if (posInFrame < curTextSize * 0.5f)
            prevTextEnd = curTextSize;
        else if (posInFrame > (frame_bb.Max.x - frame_bb.Min.x) - curTextSize * 0.5f)
            prevTextEnd = (frame_bb.Max.x - frame_bb.Min.x);
        else
            prevTextEnd = posInFrame + curTextSize * 0.5f;
    }
    ...
    for (size_t i = 0; i < count - 1; ++i)
    {
        ...
        n_frame_bb.Max.y -= sizeY;
        const char minus[1] = { '-' };
        float textSize = CalcTextSize(minus, minus + 1, NULL, true).x;
        if (textSize * 9 < minimumGapSize)
        {
            float x = (values[i] - values[0]) / scalaSize;
            float x1 = (values[i + 1] - values[0]) / scalaSize;
            uint32_t cr = (colors[i] >> 0) & 0x000000ff;
            uint32_t cg = (colors[i] >> 8) & 0x000000ff;
            uint32_t cb = (colors[i] >> 16) & 0x000000ff;
            uint32_t ca = (colors[i] >> 24) & 0x000000ff;
            ...
            cb = (cb + cb1) / 2;
            ca = (ca + ca1) / 2;
            ImU32 mixedColor = 0x0;
            mixedColor |= cr << 0;
            mixedColor |= cg << 8;
            mixedColor |= cb << 16;
            mixedColor |= ca << 24;
            n_frame_bb.Min.x = gapPosX[i] - gapSizeX[i] * 0.5f;
            n_frame_bb.Max.x = gapPosX[i] + gapSizeX[i] * 0.5f;

            ImGui::PushStyleColor(ImGuiCol_Text, mixedColor);
            RenderTextClipped(n_frame_bb.Min, n_frame_bb.Max, minus, minus + 1, NULL, ImVec2(0.5f, 0.5f));
            ImGui::PopStyleColor();
        }
    }
...
IMGUI_API void ImGui::ColorGradientBar(const char* label, size_t count, ImU32* colors, float* values, const char* format)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    ImRect b_frame_bb = frame_bb;
    float sizeX = (frame_bb.Max.x - frame_bb.Min.x);
    float scalaSize = values[count - 1] - values[0];
    for (size_t i = 0; i < count - 1; ++i)
    {
        b_frame_bb.Min.x = frame_bb.Min.x + (values[i] - values[0]) * sizeX / scalaSize;
        b_frame_bb.Max.x = frame_bb.Min.x + (values[i + 1] - values[0]) * sizeX / scalaSize;
        window->DrawList->AddRectFilledMultiColor(b_frame_bb.Min, b_frame_bb.Max, colors[i], colors[i + 1], colors[i + 1], colors[i]);
    }
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        n_frame_bb.Max.y -= sizeY;
        char value_buf[64];
        const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), ImGuiDataType_Float, (values + i), format);
        ImGui::PushStyleColor(ImGuiCol_Text, colors[i]);
        RenderTextClipped(n_frame_bb.Min, n_frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2((values[i] - values[0]) / scalaSize, 0.5f));
        ImGui::PopStyleColor();
        float curTextSize = CalcTextSize(value_buf, value_buf_end, NULL, false).x;
...
IMGUI_API bool ImGui::SliderFloatRange(const char* label, float* v, size_t rangeSliderCount, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    return SliderScalarRange(label, ImGuiDataType_Float, v, rangeSliderCount, &v_min, &v_max, format, flags);
}

IMGUI_API bool ImGui::SliderFloatRange(const char* label, float* v, ImU32* colors, size_t rangeSliderCount, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    return SliderScalarRange(label, ImGuiDataType_Float, v, colors, rangeSliderCount, &v_min, &v_max, format, flags);
}
