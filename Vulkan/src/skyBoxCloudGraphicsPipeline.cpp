...
void SkyBoxCloudGraphicsPipeline::updateData(AppResources* app, GuiControl* gui_control)
{
	...
	glm::mat4 moon_rot =
		glm::translate(cameraPos) * glm::rotate(glm::radians(40.0f), glm::normalize(glm::vec3(1.0f, 0.0f, 0.0f))) *
	...
	pushConstants.time = glm::radians(gui_control->var.cloudTimer);
	pushConstants.moonLuminance = gui_control->camera.getFar();
	pushConstants.cloudVP = glm::ortho(-cloudsSize, cloudsSize, -cloudsSize, cloudsSize, 0.0f, 500.0f) *
		glm::lookAt(cameraPos, cameraPos + glm::vec3(0.0f, 1.0f, 0.0f),
					glm::vec3(0.0f, 0.0f, 1.0f));

	glm::mat4 cloudsTrans = glm::translate(glm::vec3(cameraPos.x, -18000.0f + cloudsHeight, cameraPos.z));
	pushConstants.M = cloudsTrans * glm::scale(glm::vec3(18000.0f));


}

void SkyBoxCloudGraphicsPipeline::createPipeline(AppResources* app, bool full)
{
	vk::ShaderModule vertexShaderModule, fragmentShaderModule;
	createShaderModule(app->device, "cloud.vert", vertexShaderModule);
	createShaderModule(app->device, "cloud.frag", fragmentShaderModule);
